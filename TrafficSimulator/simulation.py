...
class Simulation:
    def __init__(self, max_gen: int = None):
        ...
        self.roads: List[Road] = []  # elenco delle strade
        self.generators: List[VehicleGenerator] = []  # generatore dei veicoli
        self.traffic_signals: List[TrafficSignal] = [] 
        self.current_weather: str = "Sunny"  # Meteo iniziale di default

        self.collision_detected: bool = False  # se è presente una collisione tra le auto 
        self.n_vehicles_generated: int = 0 # numero di veicoli generati
        self.n_vehicles_on_map: int = 0 # veicoli sulla mappa

        self._gui: Optional[Window] = None

        self._non_empty_roads: Set[int] = set()  # indici delle strade con i veicoli
        ...
        # Strade in ingresso e in uscita
        ...
        self.vehicles_out_of_map = []

    def set_weather(self, weather: str):
        """Imposta il meteo corrente nella simulazione."""
        self.current_weather = weather

    def get_weather(self) -> str:
        """Ritorna il meteo corrente."""
    ...
    def add_traffic_signal(self, roads: List[List[int]], cycle: List[Tuple],
        ...
        # cycle: regola il tempo per cui i segnali sono verdi, gialli o rossi
    ...
    def intersections(self) -> Dict[int, Set[int]]:
        """
        Filtra gli incroci per considerare solo le strade che hanno veicoli. 
    ...
    def current_average_wait_time(self) -> float:
        """ 
        Ritorna il tempo di attesa medio dei veicoli. 
    ...
    @property
    def current_average_wait_time_on_map(self) -> float:
        """ 
        Ritorna il tempo di attesa medio dei veicoli sulla mappa. """
        on_map_wait_time = 0
        if self.n_vehicles_on_map:
            total_on_map_wait_time = sum(vehicle.get_wait_time(self.t) for i in self.non_empty_roads
                                         for vehicle in self.roads[i].vehicles)
            on_map_wait_time = total_on_map_wait_time / self.n_vehicles_on_map
        return on_map_wait_time
    ...
    def run(self, action: Optional[int] = None) -> None:
        """ 
        Esegue la simulazione per n frame. Aggiorna i segnali, rileva collisioni
        e controlla se la simulazione è completata. 
    ...
    def _detect_collisions(self) -> None:
        """ 
        Confronta la posizione dei veicoli per rilevare collisioni, usando una distanza minima (raggio). 
    ...
    def _check_out_of_bounds_vehicles(self):
        ...
        #self.vehicles_out_of_map.clear()
            ...
            if lead.x >= road.length:
                ...
                if lead.current_road_index + 1 < len(lead.path):
                    ...
                    if not road.vehicles:
                        new_empty_roads.add(road.index)
                    self.n_vehicles_on_map -= 1
                    self.vehicles_out_of_map.append(lead)
                    # Update the waiting times sum
                    self._waiting_times_sum += lead.get_wait_time(self.t)
                                # Aggiungi il tempo di attesa del veicolo a una lista
                    self.vehicles_out_of_map.append({
                        'vehicle': lead,
                        'wait_time': lead.get_wait_time(self.t)
                    })
        self._non_empty_roads.difference_update(new_empty_roads)
        self._non_empty_roads.update(new_non_empty_roads)

    def get_wait_times(self):
        # Restituisce una lista con i tempi di attesa di tutti i veicoli
        wait_times = []
        for road in self.roads:
            for vehicle in road.vehicles:
                wait_times.append(vehicle.get_wait_time(self.t))  # Passa il tempo di simulazione al metodo del veicolo
        return wait_times