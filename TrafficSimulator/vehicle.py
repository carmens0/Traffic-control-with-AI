...
class Vehicle:
    ...
    def update_behavior_based_on_weather(self, weather: str):
        """
        Aggiorna la velocità massima e la distanza di sicurezza in base al meteo.
        :param weather: Condizione meteorologica attuale ("Sunny", "Rain", "Snow", "Fog").
        """
        if weather == "Rain":
            self.v_max = self._v_max * 0.8  # Riduci velocità del 20%
            self.s0 = 6  # Aumenta distanza minima
        elif weather == "Snow":
            self.v_max = self._v_max * 0.5  # Riduci velocità del 50%
            self.s0 = 15  # Aumenta notevolmente la distanza minima
        elif weather == "Fog":
            self.v_max = self._v_max * 0.7  # Riduci velocità del 30%
            self.s0 = 8  # Aumenta la distanza minima
        else:  # Sunny
            self.v_max = self._v_max  # Ripristina velocità originale
            self.s0 = 4  # Ripristina distanza originale
